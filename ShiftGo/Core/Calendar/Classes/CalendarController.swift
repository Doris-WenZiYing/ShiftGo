...
public class CalendarController: ObservableObject {
    ...
    public func navigateToNextMonth() {
        let nextMonth = yearMonth.addMonth(value: 1)
        setYearMonth(nextMonth)
    }

    public func navigateToPreviousMonth() {
        let previousMonth = yearMonth.addMonth(value: -1)
        setYearMonth(previousMonth)
    }

    public func selectDate(_ date: YearMonthDay) {
        yearMonthDay = date
    }

    public func isDateSelected(_ date: YearMonthDay) -> Bool {
        guard let selected = yearMonthDay else { return false }
